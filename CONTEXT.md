Created At: 2026-07-13T23:45:47Z
Completed At: 2026-07-13T23:45:47Z
...
**Warm / Cold**:
Whether a flow run reuses an already-booted engine process (**warm**) or forks a fresh one (**cold**). Warm is the *steady state* of a pre-sized, always-on fleet with `AP_REUSE_SANDBOX` on; cold is the *edge* — the first run after a deploy, restart, or scale-up, or any run when reuse is off. Because a **Worker** is concurrency 1, a burst beyond the warm worker count either queues or spills onto cold workers. Cold pays a one-time engine fork-and-boot that the warm path skips. The distinction is a property of single-tenant **dedicated** execution, identical on self-host and on dedicated Cloud workers — not a Cloud-vs-self-host difference.
_Avoid_: cold start as a Cloud-only term, warm cache (the on-disk provision cache is a separate thing — warm/cold here is the engine *process*, not the disk).
